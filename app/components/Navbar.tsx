...
export default function Navbar() {
  ...
  const navClass =
    `fixed top-0 left-0 w-full z-50 transition-all duration-300
     ${scrolled ? 'bg-green-700/95 py-2 shadow-lg backdrop-blur' : 'bg-green-600 py-4'}`;
    ...
    <header className={navClass}>
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
        ...
        <Link href="/" className="text-white font-bold tracking-wide">
          <span className={`transition-all duration-300 ${scrolled ? 'text-2xl' : 'text-3xl'}`}>
            MiRetiro
          </span>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-6 text-white text-lg font-medium">
          ...
          <Link href="/testimonios" className={linkClass('/testimonios')}>Testimonios</Link>
          <Link href="/sobre-mi" className={linkClass('/sobre-mi')}>Sobre nosotras</Link>
      ...
      {open && (
        <nav className="md:hidden mt-3 px-4 pb-4 text-white text-lg font-medium">
          <ul className="flex flex-col gap-2">
            ...
            <li><Link href="/faq" className={linkClass('/faq')}>Preguntas</Link></li>
